...
export const ShortAnswer = ({ currentQuestion, currentTest }: { currentQuestion: string, currentTest: string }) => {
  ...
  return (
    <>
        <p>Write short answer</p>
        <input placeholder='answer' className={styles.input} type="text" value={answers.find(el => question?.answers![0].id! === el.id)?.answer} onChange={e => writeAnswer(e.target.value, question?.answers![0].id!)} />
    </>
  )
}
